...
#============================================
# MODULE 4: AUTOMATED PAPER DRAFTING & SYNTHESIS (GPT)
#============================================
...
def setup_llm():
    ...
    print("✅ OpenRouter GPT Client Initialized")
    return client

#============================================================
# 2. LOAD MODULE 3 OUTPUT
#============================================================
def load_processed_results(filepath="data/processed_results/final_results.json"):
    if not os.path.exists(filepath):
    ...
    print(f"✅ Loaded {data['total_processed_papers']} processed papers")
    return data

#============================================================
# 3. PROMPT BUILDERS
#============================================================
def build_abstract_prompt(topic, papers):
    abstracts = []
    for p in papers:
        if "extracted_sections" in p and "abstract" in p["extracted_sections"]:
            abstracts.append(p["extracted_sections"]["abstract"][:1200])

    return f"""
You are writing a SYSTEMATIC LITERATURE REVIEW abstract.

Topic: {topic}

Based on the following abstracts from multiple papers, generate:
• Background
• Objective
• Methods overview
• Key findings
...
Write in formal academic tone (150–200 words).
"""
def build_methods_prompt(papers):
    methods_text = []
    for p in papers:
        methods_text.append(
            p.get("extracted_sections", {}).get("methods", "")[:1200]
        )

    return f"""
Write the METHODS section of a systematic review.
...
• Text extraction & NLP
• Cross-paper synthesis approach
...
"""
def build_results_prompt(papers):
    findings = []
    for p in papers:
        findings.extend(p.get("key_findings", []))

    return f"""
Write the RESULTS section of a literature review.
...
Ensure structured paragraphs and analytical tone.
"""

#============================================================
# 4. GPT TEXT GENERATION (OPENROUTER MODEL)
#============================================================
def generate_section(client, prompt, model="openai/gpt-4o-mini"):
...
def format_references_apa(papers):
    ...
    for p in papers:
        ...
        ref = f"{author_str} ({year}). {title}. arXiv. {url}"
...
#============================================================
def run_module_4(
    processed_file="data/processed_results/final_results.json",
    output_dir="data/final_draft"
):
    print("\n" + "="*80)
    print("MODULE 4: AUTOMATED DRAFT GENERATION (OPENROUTER)")
    print("="*80)

    client = setup_llm()
    data = load_processed_results(processed_file)

    topic = data["topic"]
    papers = data["papers"]

    print("🧠 Generating Abstract...")
    abstract = generate_section(client, build_abstract_prompt(topic, papers))

    print("🧠 Generating Methods...")
    methods = generate_section(client, build_methods_prompt(papers))

    print("🧠 Generating Results...")
    results = generate_section(client, build_results_prompt(papers))

    print("📚 Formatting APA References...")
    references = format_references_apa(papers)
    ...
    # Save JSON
    json_path = os.path.join(output_dir, "automated_review.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(final_draft, f, indent=4, ensure_ascii=False)

    # Save Markdown
    md_path = os.path.join(output_dir, "automated_review.md")
...
final_review = run_module_4()
print("Automated literature review generation completed.")

# ===============================
# MODULE 5: REVIEW & REFINEMENT CYCLE
# (Automated Self-Critique + Revision)
# ===============================
def build_critique_prompt(section_name, content):
    return f"""
You are an expert academic reviewer.

Review the following {section_name} section and provide:
1. Clarity issues
2. Redundancy or missing elements
3. Methodological or logical weaknesses
...
"""
def build_revision_prompt(section_name, content, critique):
    return f"""
You are revising an academic {section_name} section.
...
• Keeps length appropriate

Return ONLY the revised section.
"""
def review_and_refine(client, section_name, content):
    critique = generate_section(
        client,
        build_critique_prompt(section_name, content)
    )

    revised = generate_section(
        client,
        build_revision_prompt(section_name, content, critique)
    )
...
# ===============================
def build_quality_eval_prompt(section_name, content):
    return f"""
Evaluate the academic quality of the following {section_name} section.
...
"""
def evaluate_quality(client, section_name, content):
    return generate_section(
        client,
        build_quality_eval_prompt(section_name, content)
    )

# ===============================
# FINAL REFINEMENT PIPELINE
# ===============================
def run_revision_cycle(client, draft):
    refined = {}

    for section in ["abstract", "methods", "results"]:
        print(f"🔁 Reviewing {section.capitalize()}...")

        review = review_and_refine(
            client,
            section.capitalize(),
            draft[section]
        )
...
# ===============================
def prepare_final_report(topic, refined_sections, references):
    report = f"""
# Automated Literature Review Report
## Topic: {topic}

---

## Abstract
{refined_sections['abstract']['final']}

---

## Methods
{refined_sections['methods']['final']}

---

## Results
{refined_sections['results']['final']}
...
def generate_initial_sections():
    ...
    methods = generate_section(client, build_methods_prompt(papers))
    results = generate_section(client, build_results_prompt(papers))
    references = "\n".join(format_references_apa(papers))

    return topic, abstract, methods, results, references


def critique_and_revise(abstract, methods, results):
    client = setup_llm()

    revised_abs = review_and_refine(client, "Abstract", abstract)["revised"]
    revised_meth = review_and_refine(client, "Methods", methods)["revised"]
    revised_res = review_and_refine(client, "Results", results)["revised"]

    return revised_abs, revised_meth, revised_res


def save_final_report(topic, abstract, methods, results, references):
    os.makedirs("data/final_draft/refined", exist_ok=True)

    filename = f"final_literature_review_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    filepath = os.path.join("data/final_draft/refined", filename)

    with open(filepath, "w", encoding="utf-8") as f:
        f.write(f"""
# Automated Literature Review: {topic}
...
""")
    ...
    return f"Final report saved successfully to:\n{filepath}"
...
with gr.Blocks(title="Automated Literature Review") as demo:

    # Header
    gr.Markdown(
        """
        # **Automated Literature Review: stock price**
    ...
    topic, abs_txt, meth_txt, res_txt, ref_txt = generate_initial_sections()

    topic_state.value = topic
    ...
    with gr.Tabs():
        ...
        with gr.Tab("References"):
            references_box = gr.Textbox(
                value=ref_txt,
                label="APA References",
                lines=18,
                interactive=False
            )
    ...
    # ---------------- BUTTONS ----------------
    with gr.Row():
        revise_btn = gr.Button("Critique/Revise", variant="primary")
    ...
    revise_btn.click(
        ...
        outputs=[abstract_box, methods_box, results_box]
    )

    save_btn.click(
        save_final_report,
        inputs=[topic_state, abstract_box, methods_box, results_box, references_box],
        outputs=save_status
    )
...
def export_pdf(topic, abstract, methods, results, references):
    os.makedirs("data/final_draft/refined", exist_ok=True)
    pdf_path = f"data/final_draft/refined/final_review_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"

    styles = getSampleStyleSheet()
    doc = SimpleDocTemplate(pdf_path)

    content = [
        Paragraph(f"<b>Automated Literature Review: {topic}</b>", styles["Title"]),
        Paragraph("<b>Abstract</b>", styles["Heading2"]),
        Paragraph(abstract, styles["Normal"]),
        Paragraph("<b>Methods</b>", styles["Heading2"]),
        Paragraph(methods, styles["Normal"]),
        Paragraph("<b>Results</b>", styles["Heading2"]),
        Paragraph(results, styles["Normal"]),
        Paragraph("<b>References</b>", styles["Heading2"]),
        Paragraph(references.replace("\n", "<br/>"), styles["Normal"])
    ]

    doc.build(content)
    return pdf_path
def evaluate_all_sections(client, abstract, methods, results):
    return (
        evaluate_quality(client, "Abstract", abstract),
        evaluate_quality(client, "Methods", methods),
        evaluate_quality(client, "Results", results),
    )
...
def full_pipeline(topic_input):
    client = setup_llm()
    data = load_processed_results()

    papers = data["papers"]
    topic = topic_input if topic_input else data["topic"]
    ...
    qa, qm, qr = evaluate_all_sections(client, abstract, methods, results)

    return topic, abstract, methods, results, references, qa, qm, qr


def revise_pipeline(old_abs, old_meth, old_res):
    client = setup_llm()

    r_abs = review_and_refine(client, "Abstract", old_abs)["revised"]
    r_meth = review_and_refine(client, "Methods", old_meth)["revised"]
    r_res = review_and_refine(client, "Results", old_res)["revised"]

    return (
        highlight_changes(old_abs, r_abs),
        highlight_changes(old_meth, r_meth),
        highlight_changes(old_res, r_res),
    )


with gr.Blocks(theme=gr.themes.Soft(), title="Automated Literature Review") as demo:

    gr.Markdown("# 📘 Automated Literature Review System")

    # ---------- USER INPUT ----------
    topic_input = gr.Textbox(
        label="Research Topic",
        placeholder="Enter topic (e.g., Stock Price Prediction)",
    )
    ...
    with gr.Tabs():
        with gr.Tab("Abstract"):
            ...
            abstract_score = gr.Textbox(label="Quality Evaluation")

        with gr.Tab("Methods"):
            methods_box = gr.Markdown()
            methods_score = gr.Textbox(label="Quality Evaluation")

        with gr.Tab("Results"):
            results_box = gr.Markdown()
            results_score = gr.Textbox(label="Quality Evaluation")

        with gr.Tab("References"):
            references_box = gr.Textbox(lines=15)
    ...
    pdf_btn.click(
        export_pdf,
        inputs=[topic_input, abstract_box, methods_box, results_box, references_box],
        outputs=pdf_status,
    )
